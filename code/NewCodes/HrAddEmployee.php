...
$employee_id = $prefix . str_pad($max_id, 4, '0', STR_PAD_LEFT);
    ...
    try {
        ...
        if ($stmt->execute()) {
            ...
            $new_employee_id = $conn->insert_id; // This won't work for the ID
            
            // We need to get the actual Employee_ID that was generated by the trigger
            $get_emp_id_query = "SELECT Employee_ID FROM Employee 
                                WHERE Email = ? LIMIT 1";
            $get_emp_stmt = $conn->prepare($get_emp_id_query);
            $get_emp_stmt->bind_param("s", $email);
            $get_emp_stmt->execute();
            $emp_result = $get_emp_stmt->get_result();
            
            if ($emp_row = $emp_result->fetch_assoc()) {
                $new_employee_id = $emp_row['Employee_ID'];
                ...
                throw new Exception("Failed to retrieve the generated Employee_ID");
            }
        } else {
            // Rollback on failure
            $conn->rollback();
